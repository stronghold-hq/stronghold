...
func TestCreateAPIKey(t *testing.T) {
	...
	key, err := db.CreateAPIKey(ctx, account.ID, "sk_live_abc1", "sha256hashvalue", "My Key")
	require.NoError(t, err)
...
func TestGetAPIKeyByHash(t *testing.T) {
	...
	created, err := db.CreateAPIKey(ctx, account.ID, "sk_live_xyz", "hash_lookup_test", "Lookup Key")
	require.NoError(t, err)
...
func TestGetAPIKeyByHash_Revoked(t *testing.T) {
	...
	key, err := db.CreateAPIKey(ctx, account.ID, "sk_live_rev", "hash_revoked_test", "Revoked Key")
	require.NoError(t, err)
...
func TestListAPIKeys(t *testing.T) {
	...
	_, err := db.CreateAPIKey(ctx, account.ID, "sk_live_a", "hash_a", "Key A")
	require.NoError(t, err)

	_, err = db.CreateAPIKey(ctx, account.ID, "sk_live_b", "hash_b", "Key B")
	require.NoError(t, err)

	keyToRevoke, err := db.CreateAPIKey(ctx, account.ID, "sk_live_c", "hash_c", "Key C (revoked)")
	require.NoError(t, err)
...
func TestRevokeAPIKey(t *testing.T) {
	...
	key, err := db.CreateAPIKey(ctx, account.ID, "sk_live_del", "hash_del", "Key To Revoke")
	require.NoError(t, err)
	assert.Nil(t, key.RevokedAt)
...
func TestRevokeAPIKey_WrongAccount(t *testing.T) {
	...
	key, err := db.CreateAPIKey(ctx, account1.ID, "sk_live_own", "hash_own", "Owner's Key")
	require.NoError(t, err)
...
func TestUpdateAPIKeyLastUsed(t *testing.T) {
	...
	key, err := db.CreateAPIKey(ctx, account.ID, "sk_live_lu", "hash_lu", "Last Used Key")
	require.NoError(t, err)
	assert.Nil(t, key.LastUsedAt)
...
func TestCountActiveAPIKeys(t *testing.T) {
	...
	// Create 3 keys
	_, err = db.CreateAPIKey(ctx, account.ID, "sk_live_c1", "hash_c1", "Key 1")
	require.NoError(t, err)
	_, err = db.CreateAPIKey(ctx, account.ID, "sk_live_c2", "hash_c2", "Key 2")
	require.NoError(t, err)
	keyToRevoke, err := db.CreateAPIKey(ctx, account.ID, "sk_live_c3", "hash_c3", "Key 3")
	require.NoError(t, err)
